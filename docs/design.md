...
- ~~Session persistence~~ — **done**: `internal/session` saves the conversation (`[]agent.Message`) after every turn, scoped per working directory so separate projects don't share history. `main.go` loads the most recent one back on startup (`session.LoadLatest`) and `tui.New` reconstructs the visible transcript from it (`internal/tui/history.go`), skipping the permission-prompt step since every replayed tool call was already resolved in a past run. Deliberately doesn't restore which model was selected — that's a per-run choice (`CHISEL_MODEL`/default), not part of "the conversation". Superseded by named/multiple sessions per directory — see the Phase 4 bullet below; `/new` no longer abandons anything, it mints a fresh session.
...
- **Finishing Phase 3** (2026-07, directly after the review above): asked explicitly to close out the three items the review had left open or demoted — custom slash commands, checkpoint/rewind, and the todo list — reversing its own prioritization once told to build all three anyway. The riskiest of the three, checkpoint/rewind, got its correctness verified the same way as the earlier concurrency bugs: not just written and hoped for, but tested against a real git subprocess, including a specific test proving that rewinding *past* a state doesn't destroy it — a later `Restore` back to that same commit hash still works, because git commits are append-only and `reset --hard` only moves what the working tree currently reflects, never what's recorded in the shadow repo's history.
- **Scoping Phase 4** (2026-07): asked to "implement all Phase 4," which — unlike Phase 3's finish-the-list ask — got a scope check first rather than an attempt to build all six items. Three of them (sandboxing, a second provider, IDE integration) are conditional in the roadmap's own wording ("if it turns out to matter," "if one ever genuinely earns its way in") and a fourth (image input) is gated on a fact (a vision-capable model in the catalog) that hadn't been checked — building any of the four without a concrete target or trigger condition actually met would mean guessing at requirements, the exact mistake chisel's own multi-provider history (§3) already paid for once. Asked which to skip explicitly rather than silently picking for the user; all four were confirmed skip. The remaining two — skill files and background tasks — were built the same way as everything else here: real subprocess tests throughout, including one that starts an actual `sleep 30` in the background and confirms it's dead within 5 seconds of the exit-cleanup path, not just that the code *should* kill it.
- **Feature parity with mainstream CLI agents** (2026-07): a second Fable review, this time researching what Claude Code, Codex CLI, Gemini CLI, Crush, Goose, and others actually ship, cross-checked against what chisel already had rather than assumed. Seven gaps came back, built simplest-first: automatic compaction at the context threshold (chisel already had `/compact`'s logic, just needed to trigger it instead of only suggesting it); bang mode (`!command` runs directly through the persistent bash session, no model round-trip, no permission prompt — it's the user's own direct action); a doom-loop guard (three identical tool calls in a row forces a confirmation even for an otherwise-auto-allowed call — OpenCode's own term for a real, observed tool-calling failure mode, not a hypothetical one); `/usage`; persistent glob-pattern permission rules (`.chisel/permissions.json`, trust-gated like hooks); and structured JSON headless output (`chisel -p -json`).
  - `/usage` is the one item that got *scoped down* rather than built as originally framed: the plan was estimating spend against OpenCode Go's hard $12/5hr, $30/week, $60/month caps, but verified directly against the live API before writing any code — both a one-word request and a ~150-word one come back with the trailing "cost" bookkeeping frame reading exactly `"0"`, and there's no separate account/usage endpoint either. This subscription just doesn't expose real dollar figures to estimate from, so `/usage` reports token/request counts instead, with an explicit note on why no dollar estimate is shown — fabricating one would be worse than not showing one at all, the same reasoning already applied to not maintaining a per-model context-window table.
  - The seventh — LSP-backed diagnostics and references — took the biggest pivot of the pass. The plan going in was a hand-rolled `internal/lsp` package: JSON-RPC-over-stdio framing, an `initialize` handshake, position encoding, `textDocument/didOpen` plus waiting on asynchronous `publishDiagnostics` notifications, two new tools. That package got written (wire framing, tested against a real in-memory pipe) — then, before going further, `gopls -h` turned up a `mcp` subcommand: gopls can run *as an MCP server itself*, confirmed live by hand-driving its stdio JSON-RPC and finding it hands back seven real tools (`go_diagnostics`, `go_symbol_references`, `go_search`, and more) — everything the plan wanted, plus more, with a *symbol name* as input rather than a raw line/column position a model would have to compute. Since chisel already has a complete, tested MCP client, the hand-rolled LSP package was deleted entirely in favor of `main.go` auto-registering `gopls mcp` as an MCP server whenever a `go.mod` is present and `gopls` is on `PATH` — reusing `internal/mcp`'s existing server-spawning, tool-discovery, and permission-prompt code completely unchanged. Deliberately *not* given hand-written-tool-style auto-allow treatment: MCP tools always ask (chisel can't audit an arbitrary server's tools), and `go_rename_symbol` genuinely does mutate files, so the uniform rule is correct here rather than a gap. Also surfaced, via the same live testing, a real nil-map panic in the new `Registry.AddServer` (a bare `&Registry{}`, as opposed to one built through `LoadAndStartAll`, has a nil `servers` map) — fixed and covered by its own regression test rather than just working around it in the one call site that happened to trip it.
...
- ~~Custom slash commands~~ — **done**: a new `internal/customcmd` package loads Markdown files as canned prompts, the same two-layer convention as `internal/memory` — user-scoped (`~/.chisel/commands/*.md`) and project-scoped (`<workDir>/.chisel/commands/*.md`), a project-level command overriding a user-level one of the same name. Filename (minus `.md`) becomes the command name; `$ARGUMENTS` in the template is substituted with whatever followed the command name, or appended if the template doesn't reference `$ARGUMENTS` at all. Deliberately no folder-trust gate unlike hooks: a custom command is canned prompt text, not code that executes automatically — invoking one sends that text through the exact same `submitText` path as anything typed directly, so whatever the model does in response still goes through the normal permission gate.
- ~~Checkpoint/rewind~~ — **done**, reversing the earlier demotion once explicitly asked for: a new `internal/checkpoint` package snapshots the working directory into a shadow git repo kept entirely outside the project (`~/.chisel/checkpoints/<hash>/repo.git`, `--work-tree` pointed at the real project directory — the standard technique for a git repo whose metadata lives separately from what it tracks). `.gitignore` is honored automatically since it's read from the work-tree regardless of which `--git-dir` is active; the project's own `.git` and chisel's own `.chisel` are always excluded via pathspec. A checkpoint is taken asynchronously at the start of every turn (concurrently with the model request — nothing can modify a file until at least one tool call resolves, which is always a full round-trip away, so there's no need to block on this). `/rewind [n]` lists checkpoints or restores to one; `/rewind confirm` is a required second step before anything destructive happens, since restoring overwrites real files. `Restore` always checkpoints the current state first (labeled "before rewind") before `git reset --hard`ing backwards — proven live in a test that a state rewound *past* remains recoverable by its own commit hash, since git commits are append-only and nothing is actually destroyed, just no longer on the checkpoint list's current line of history. Restoring also truncates `m.messages` and rebuilds the transcript via the same `renderHistory` used for resuming a saved session, so code and conversation move together.
- ~~Todo list~~ — **done**, same reversal: a new `update_todos` tool (`internal/agent/todo.go`) lets the model maintain a checklist — content + status (pending/in_progress/completed), always the full list per call. Needs no permission (falls through `NeedsPermission`'s default, like glob/grep): it only affects in-memory UI state, nothing on disk or in a shell. The tool's own result sent back to the model is just a short confirmation string; the structured list lives only in the call's own arguments, which `internal/tui/todo.go`'s `parseTodos` reads directly in `handleToolResult` rather than threading through `ToolResult`. Rendered as a persistent block between the transcript and the input box, not appended to the transcript itself — `Model.recomputeViewportHeight` accounts for its variable height on every change, not just on resize, so the layout never falls out of sync with what's actually being shown.
...
- Sandboxing the bash tool (container or OS-level sandbox) — non-negotiable once there's an auto-approve/"just do it" mode. Skipped deliberately in the pass that did the rest of this list: its own trigger condition isn't met yet (no auto-approve mode exists), and the mechanism (bubblewrap vs. Docker vs. something else) is a real decision, not a default to just pick
- A second provider, if one ever genuinely earns its way in — would mean reintroducing a thin `Provider` interface, deliberately not built speculatively this time either. Skipped for exactly that reason: there's no second provider to actually plug into it yet, and building the interface first would be guessing at its shape
- ~~User-defined skill files~~ — **done**: a new `internal/skill` package loads Markdown files as skills — user-scoped (`~/.chisel/skills/*.md`) and project-scoped (`<workDir>/.chisel/skills/*.md`), the same two-layer convention as memory and custom commands. Each file has a minimal frontmatter `description:` line plus a body; only the name and description ever reach the system prompt (`Client.SetSkills`), keeping an unloaded skill's cost to one line — the full body only reaches the model through a new `load_skill` tool, added to the tool set only once at least one skill exists. `load_skill` needs no permission: it just reads a local file the user placed there themselves.
- ~~Background/async task execution~~ — **done**: a new `bash_background` tool starts a shell command as a standalone process (not through the persistent `BashSession` — that session's single-owner-access contract would be violated by a second concurrent caller) with its own context, deliberately independent of the turn that started it, so it isn't cancelled by esc or the turn ending. Intercepted in `internal/tui`'s `executeTool` before it would otherwise reach `agent.Execute` — the same interception point MCP calls already use there — since it needs to return a `tea.Batch` (an immediate "started" result, a record for `Update` to track, and a watcher that fires whenever the command actually finishes) rather than one synchronous result. On completion, its output is folded into the conversation as a synthetic message (so the model picks it up on its next turn with no separate "check on it" tool needed) and the user is notified via the same bell/OSC9 mechanism a permission prompt uses. `Setpgid` plus a `cmd.Cancel` override that kills the whole process group (mirroring `BashSession.stop`) means a background command that spawns its own children doesn't orphan them when cancelled — verified live: a real `sleep 30` killed within 5 seconds of calling the exit-cleanup hook, not left running to completion.
- Lightweight IDE integration, if it turns out to matter for daily use. Skipped: no chosen target (which IDE, which protocol) and no evidence yet that it's actually needed for daily use
...
- ~~Named/multiple sessions per directory~~ — **done**, the largest single change from a second Fable-model review looking specifically for gaps left after the first: `internal/session` was rewritten from one file per working directory to one *directory* of session files per working directory (`session.List`/`LoadByID`/`LoadLatest`/`NewID`), with transparent migration of the old flat-file format on first read. `/sessions` lists every saved session for the current directory, most-recent-first; `/resume <n>` switches the running conversation to one. `/new` no longer deletes anything — it mints a fresh `session.NewID()` and the abandoned conversation stays resumable. Both `/new` and `/resume` now save immediately (even an empty, just-started session) rather than waiting for a turn to complete, specifically so quitting right after either one still resumes the right session next launch — which only works because a zero-message session is treated as a real, listable one, not silently equivalent to "nothing saved."
- Also from that same review, in roughly descending order of how much it mattered: a background task's output is now buffered rather than spliced into the conversation immediately if it completes mid-turn (`internal/tui/background.go`), avoiding a shape the API rejects; retry-with-backoff on transient 429/5xx errors (`internal/agent/client.go`'s `doWithRetry`); persistent "always allow" permission rules (a "p" option alongside the plain y/n prompt, written to `.chisel/permissions.json`); input history surviving restarts plus ctrl+r reverse search (`internal/history`); project-scoped `.chisel/mcp.json` alongside the user-scoped one, trust-gated the same way hooks are; `chisel -p` reading piped stdin; a `--version` flag; MCP `tools/list` pagination (was silently truncating a paginated server's tools to its first page) with a page cap against a server whose cursor never terminates; a corrupt session file now renamed after warning once instead of re-warning on every future launch forever; age-based pruning of whole checkpoint shadow repos (`checkpoint.PruneStaleRepos`), not just a per-project commit-count cap; and the status bar showing the current git branch and a dirty marker, refreshed after anything that can change it (a turn, a bang command, a rewind, a background task) rather than only once per model turn.
- Image input — attach a screenshot or design spec to a message (Codex and Claude Code both take pasted images); gated on whether OpenCode Go's catalog actually serves a vision-capable model (`mimo-v2-omni` suggests maybe). Skipped: not verified against the live catalog, and building the plumbing without a model to test it against is speculative

### Phase 5 — Feature-parity sweep against other CLI agents (2026-07)

A Fable-model research pass compared chisel against Claude Code, opencode (the SST open-source terminal agent — a different thing from OpenCode Go, chisel's own API provider), Aider, Codex CLI, Cursor, Amp, Goose, and others, looking specifically for UX patterns and capabilities several tools converged on independently rather than one-off features. Explicitly excluded anything already covered by Phase 4's declined list (sandboxing, a second provider, IDE integration, image input) and two items scoped down after a check-in — voice input (no STT backend chisel has any reason to add yet) and Claude-Code-style ghost-text input suggestions (chisel's terminal-only TUI has no IDE-level context to predict from; the command palette and `@file` completion already cover the well-scoped version of this). Twelve items were built:

- ~~Accept-edits permission mode~~ — **done**: `agent.Mode` widened from plan mode's original bool to a three-way enum (`ModeNormal`/`ModeAcceptEdits`/`ModePlan`), reusing the exact `decidePermission` plumbing plan mode already had rather than a second independent flag. `/accept-edits` auto-approves only `str_replace_based_edit_tool` (still diffed, still confined to `resolveInWorkDir`) — bash and MCP calls are entirely unaffected and still ask every time, deliberately narrower than a general auto-approve mode, since chisel still has no bash sandbox and Phase 4's own reasoning for deferring that stands unchanged. Switching into plan mode or accept-edits always wins over the other, since they're mutually exclusive states of the one `Mode`, not two independent toggles.
- ~~`AGENTS.md` interop~~ — **done**: `internal/memory.Load` now also reads a project's `AGENTS.md` (opencode, Codex CLI, and Amp have all independently converged on this filename) as an additional layer alongside `CHISEL.md`, not instead of it — a repo already using one of those tools no longer gets silently ignored just because it isn't named `CHISEL.md`. The startup/`/status` banner text was reworded to stop claiming "CHISEL.md (project)" unconditionally, since the project layer might have come from `AGENTS.md` instead.
- ~~Agent-writable persistent memory~~ — **done**: a new `internal/agentmemory` package and `remember` tool let the model persist a short note to `<workDir>/.chisel/MEMORY.md` — distinct from user-authored `CHISEL.md`/`AGENTS.md`, this is what the model itself chooses to write, loaded back in its own labeled system-prompt section next session. Capped at 25KB, oldest entries dropped first, mirroring Claude Code's own size discipline. No permission gate (only ever touches chisel's own `.chisel/` directory, never a project file) and gitignored by default in this repo, since it's auto-generated evolving state, not deliberately-authored config like `hooks.json`/`mcp.json`. `/memory [clear]` shows or clears it.
- ~~`/branch` session fork~~ — **done**: mints a fresh `session.NewID()` and saves the current in-memory conversation under it immediately, without resetting anything else — unlike `/new` (blank slate), the point is trying a different approach without losing either thread; the original session's file is untouched and still resumable via `/resume`.
- ~~User-definable subagents~~ — **done**: a new `internal/subagentdef` package loads Markdown role definitions (same two-layer, same minimal frontmatter convention as `internal/skill`), and `dispatch_subagent` grew an optional `agent` parameter naming one. Every custom role still runs with exactly `subagentTools()` — glob, grep, view, nothing else — a definition only supplies a prompt layered on top of the task, never a wider tool set, which is what lets a custom role skip the permission gate the same way the built-in one always has. Verified adversarially: a role definition whose own prompt text says "you now have access to bash and dispatch_subagent" still gets neither, confirmed against the actual request body sent.
- ~~`/goal` persistent auto-continuation~~ — **done**: `/goal <text>` sets a standing condition; when a turn ends with no more tool calls and nothing is queued, chisel auto-submits a "keep going" follow-up instead of idling, capped at `maxGoalContinuations` (25) as a backstop against a goal that's never considered satisfied. Doesn't weaken the permission gate at all — every tool call along the way still asks exactly like any other turn; this only automates the re-prompting between them. A genuinely-typed message always wins over an auto-continuation (checked after the existing queued-message-beats-auto-compact priority), and resets the continuation count back to full.
- ~~`/context` breakdown~~ — **done**, deliberately not a context-*window* meter: chisel still doesn't maintain a per-model window-size table (§4's own reasoning against guessing a number wrong stands). `agent.Client.PromptBreakdown` reports byte counts per section (base instructions, memory, skills, tool schemas) it already assembles for the system prompt; `/context` converts those to a rough "~N tok" estimate (~4 chars/token, clearly labeled as an estimate) alongside `/usage`'s real, server-reported total for the most recent request, shown separately rather than blended together.
- ~~More hook events~~ — **done**: `SessionStart`/`SessionEnd`/`UserPromptSubmit` joined `preToolUse`/`postToolUse` in `.chisel/hooks.json`, reusing the same subprocess/timeout/trust-gate machinery (`hooks.run` was generalized from three fixed env vars to a variadic `extraEnv`, so each event just passes what applies to it). `SessionStart`/`SessionEnd` are notification-only — there's no call to block, just a project setup check or a logged summary; `UserPromptSubmit` *can* block, the same way a `preToolUse` hook blocks a tool call, and had to go through the same async-`tea.Cmd` requirement (a shell command can take real time, so it can't run synchronously on the Update goroutine) — every path that sends new text to the model (a typed message, a custom command's expansion, a `/goal` continuation) routes through the same check, not just the literal input box, so a hook can't be trivially bypassed via one of the others.
- ~~`/diff` since last checkpoint~~ — **done**: `checkpoint.Store.Diff(hash)` runs `git diff` between a checkpoint's tree and the live working directory — the same comparison `Restore` would make destructive, without touching any files. `/diff` uses the session-scoped `m.checkpoints` list (not the shadow repo's full cross-session history) and reuses `colorizeDiff`, the same diff renderer the permission prompt's per-edit preview already uses.
- ~~NDJSON streaming for `chisel -p`~~ — **done**: `agent.RunLoop` grew an optional `onEvent func(LoopEvent)` callback, fired around every tool call (start/end) — nil for every existing caller (`RunSubagent` never streams anywhere), and headless mode's new `-json-stream` flag is the one place that supplies one, printing each `tool_call` line as it happens plus a final `result` line, both tagged with an explicit `"type"` so a line-oriented parser doesn't need to already know which is last.
- ~~Planner/executor two-model split~~ — **done**: `agent.Client.SetPlannerModel`/`CHISEL_PLANNER_MODEL` (mirroring Goose's `GOOSE_PLANNER_MODEL`) name a second model used instead of the primary one for requests sent while in `ModePlan` — two model IDs from the one provider chisel already talks to, not a second provider abstraction. `EffectiveModelName()` (what a request right now would actually use) is distinct from `ModelName()` (always the primary) precisely because the `/model` picker and `/model <name>` need the latter regardless of mode, while the status bar and `dispatch_subagent`'s own model choice need the former. `/model planner [name|clear]` changes it mid-session.
- ~~Lint-after-edit discoverability~~ — **done**: the capability already existed via a plain `postToolUse` hook (Aider ships this as a named `--auto-lint` feature; chisel's version is just an unlabeled use of a mechanism it already had) — the gap was that nobody would think to configure one from scratch. `/hooks init` scaffolds `.chisel/hooks.json` with a real, working example (`gofmt` on whatever `str_replace_based_edit_tool` just touched, harmless no-op on a non-Go project) rather than adding a new mechanism; refuses to overwrite an existing file.
...
- **Highest-leverage next investment:** Phases 3 and 4 are fully done, and Phase 5's twelve-item feature-parity sweep is too. What's left everywhere (sandboxing, a second provider, IDE integration, image input) is all conditional on something that hasn't happened yet — an auto-approve mode, a specific provider or IDE actually earning its way in, a verified vision-capable model — rather than being ready to build today.
