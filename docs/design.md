...
- **Highest-leverage next investment:** Phase 3 is fully done. Phase 4's skill files and background tasks are done too; what's left (sandboxing, a second provider, IDE integration, image input) is all conditional on something that hasn't happened yet — an auto-approve mode, a specific provider or IDE actually earning its way in, a verified vision-capable model — rather than being ready to build today.
